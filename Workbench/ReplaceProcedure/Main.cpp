...
#include "Craft/Code/MssqlScanner.h"
...
bool ReplaceProcedureName(int nReadLine,
                         ...
                         const TSqlProcedure& oProcedure,
                         WSTRING& sOldName,
                         WSTRING& sNewName)
{
    bool bResult = false;
...
void AddProcedureArgument(wchar_t* wcsBuffer, wofstream& gOuptuFileStream, TSqlProcedure& oProcedure, const CSqlArgument& oLastArgument1, WSTRING& sInsertArgument1)
{
    wstring sLine(wcsBuffer);
...
void main()
{
    ...
    for(int i = 0; i < gInputPaths.size(); i++)
    {
        ...
        TQueried<TSqlProcedure> aQueried = oFile.QueryProcedure(sOldSpName);
        vector<TSqlProcedure> gArray = aQueried.Array();
